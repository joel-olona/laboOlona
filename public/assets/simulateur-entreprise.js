...
if (typeof $ !== 'undefined') {
    $(function () {
        initEntrepriseFormStep($("#example-company"), "h3", "section", "fade", $('#example-company').data('can-apply'));
        
        // Sélectionner le champ select
        var selectType = $('#simulateur_entreprise_type');
        ...
        // Écoute des changements sur les boutons radio et mise à jour de l'explication
        $('input[name="simulateur_entreprise[status]"]').change(function() {
            updateExplanation(this.value);
        });

        // Mise à jour initiale pour la sélection par défaut
        updateExplanation($('input[name="simulateur_entreprise[status]"]:checked').val());
    })
} else {
    console.error('jQuery not loaded');
}
